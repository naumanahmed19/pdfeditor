...
import { markDocClosed } from "./lib/persist";
...
describe("opening an already-open file", () => {
  ...
  it("keeps a tab in place when it becomes active", async () => {
    let clock = 100;
    const now = vi.spyOn(Date, "now").mockImplementation(() => ++clock);
    const file = (name: string, lastModified: number) => {
      const value = new File([new Uint8Array(16)], name, {
        type: "application/pdf",
        lastModified,
      });
    ...
    await act(async () => {
      await store.openFile(file("first.pdf", 1));
      await store.openFile(file("second.pdf", 2));
    });
    const before = store.recentFiles.map((item) => item.id);
    const first = store.recentFiles.find((item) => item.name === "first.pdf")!;

    act(() => store.switchTab(first.id));

    expect(store.activeTabId).toBe(first.id);
    expect(store.recentFiles.map((item) => item.id)).toEqual(before);
    now.mockRestore();
  });

  it("closes every open document at once", async () => {
    const file = (name: string, lastModified: number) => {
      const value = new File([new Uint8Array(16)], name, {
        type: "application/pdf",
        lastModified,
    ...
    const openIds = store.recentFiles.map((item) => item.id);
    ...
    expect(markDocClosed).toHaveBeenCalledTimes(2);
    expect(markDocClosed).toHaveBeenCalledWith(openIds[0]);
    expect(markDocClosed).toHaveBeenCalledWith(openIds[1]);
  });
});
